...
- 吸附高度规则：每次任务开始需先向用户确认吸附高度；若用户未指定，则默认使用 `2.0 Å`。
...
### 双机调度（GO_HybridDrain）
- 命名：**GO_HybridDrain**（中文：双机调度）。
- 核心策略：
  1) 先读取 young.ng 当前空闲节点，优先把任务塞满 young.ng；
  2) 超出 young.ng 容量的任务提交到 young 排队；
  3) 每当 young.ng 完成 1 个任务，就把 young 中 1 个未启动（排队）任务迁移到 young.ng。
- 推荐资源模板：
  - young.ng：每任务 3 节点、120 核（40×3）；
  - young：每任务 128 核（2 个完整节点）。
